...
def main():
	...
	for item in schema["Details"]:
		...
		for item in mgmtdata["HypervisorList"]:
			tmpDict = {"Hypervisor": item["IP"], "List": []}
			item["VMList"].append(tmpDict)
		with open('schema.json') as h:
			json.dump(schema, h,indent = 4)
		for i in range(0,num_vms):
			newVMName = item["SubnetName"] + mgmtdata["VMPrefix"] + str(i+1)
			NSName = schema["TName"]
			subprocess.call(shlex.split('./scale_up.sh ' + mgmthypip + ' ' + mgmthypuser + ' ' + newVMName + ' ' + item["SubnetName"] + ' ' + mgmtdata["ControllerNetworkName"] + ' ' + mgmtdata["ControllerIP"] + ' ' + NSName + ' ' + (i+1) + ' ' + mgmtdata["vethIP"] + ' ' + mgmthypip + ' ' + mgmthypuser))
